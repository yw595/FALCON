...
outputDirs = {'outputSmallNormal', 'outputSmalliMAT', 'outputSmallGIMME', 'outputSmalliMATMachado', 'outputSmallGIMMEMachado'};

load specificModeliMAT.mat;
load specificModelGIMME.mat;
load specificModeliMATMachado.mat;
load specificModelGIMMEMachado.mat;
models = {origRecon2, specificModeliMAT, specificModelGIMME, specificModeliMATMachado, specificModelGIMMEMachado};
...
for i=1:length(inputDirs)
	...
	for k=1:10
    	if ~isempty(regexp(inputFiles(k).name, '.csv$'))
       	   runFALCONStripped(constrainMediumExc(initializeRecon2(models{j})), [inputDir filesep inputFiles(k).name], 1, [inputDir filesep outputDir]);
    	end
    end
end
end